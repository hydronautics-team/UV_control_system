...
void CS_ROV::regulators()
{
    ...
    if (pultProtocol->rec_data.cSMode == e_CSMode::MODE_HANDLE) { //САУ тогда разомкнута
    ...
    else if (pultProtocol->rec_data.cSMode==e_CSMode::MODE_AUTO) {
        if (pultProtocol->rec_data.controlContoursFlags.yaw > 0){
        ...
        if (pultProtocol->rec_data.controlContoursFlags.roll > 0) {//крен замкнут и это его регулятора
            //1. Добавка от задающего воздействия
            X[358][0] = K[358]*X[93][0];
            X[357][0] = (X[93][0] - X[93][1])/dt; //производная от управляющего сигнала
            X[359][0] = X[357][0]*K[357];
            X[360][0] = X[358][0] + X[359][0];

            //интегральная часть
            X[353][0] = X[93][0] - X[303][0];
            X[354][0] = X[353][0]*K[353];
            X[355][0] = X[355][1] + 0.5*(X[354][0] + X[354][1])*dt; //выходное значение интегратора без полок

            if (K[354] != 0){//значит заданы полки
                X[355][0] = saturation(X[355][0],K[354],K[355]); //выходное значение интегратора с полками
            }
            X[355][1] = X[355][0];
            X[354][1] = X[354][0];

            //пропорциональная часть
            X[352][0] = X[351][0]*K[351];

            X[356][0] = X[352][0]+X[355][0];

            X[361][0] = X[356][0]+X[360][0];

            //скоростная часть

            //апериодический фильтр по скорости
            if (K[362]!=0) { //если постоянная времени не ноль, то фильтруем
                X[362][0] = X[362][1]+dt*(1/K[362])*(X[304][0]-X[362][1]); //K[362] - T фильтра
                X[362][1] = X[362][0];
            }
            else {
                X[362][0] = X[304][0];
            }
            X[363][0] = K[363]*X[362][0];
            X[364][0] = X[361][0]-X[363][0];

            //апериодический фильтр по скорости
            if (K[365]!=0) { //если постоянная времени не ноль, то фильтруем
                X[365][0] = X[365][1]+dt*(1/K[365])*(X[304][0]-X[365][1]); //K[365] - T фильтра
                X[365][1] = X[365][0];
            }
            else {
                X[365][0] = X[304][0];
            ...
            X[367][0] = (X[365][0] - X[365][1])*dt;
            X[368][0] = X[367][0]*K[367];

            X[369][0] = X[364][0] - X[368][0];


            if (K[369]!=0) { //если постоянная времени не ноль, то фильтруем
                X[370][0] = X[370][1]+dt*(1/K[369])*(X[369][0]-X[370][1]); //K[369] - T фильтра
                X[370][1] = X[370][0];
            }
            else {
                X[370][0] = X[369][0];
            ...
            X[371][0] = X[370][0]*K[370];

            X[103][0] = saturation(X[371][0],K[372],-K[372]);
